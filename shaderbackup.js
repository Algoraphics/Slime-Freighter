...
void main() {
  ...
  growval += (numrows + ((sin(time/5.0) + 1.0) / 2.0)) * growsine;
  ...
  st[0] += (1.0 - slideaxis) * slideval;
  st[1] += slideaxis * slideval;

  st = fract(st);
  float b = box(st,vec2(width, height),0.001);
  float box = b * (1.0 - reverse) + (1.0 - b) * reverse;
  boxcolor1 = color1 * box * usecolor1;
  boxcolor2 = color2 * box * usecolor2; 

  vec2 uv2 = vUv;

  float rate = 0.05 * speed * colorslide;
  ...
  float oscillate = cos(time/2.0);
  float combo = (1.0 - usecolors) * (oscillate * 0.5 
                                  + israinbow * 0.7 
                                  + usecolor1 * 0.8 
                                  + usecolor2 * 0.5)
                 + oscillate * usecolors;
